...
class Horse {
    ...
    public static function saveOddsbyRefH($refH, $hOdds, $refRace){
        try {
            $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare('UPDATE _horse_tb SET _hodds = :hodds WHERE _id_horse=:refH AND _ref_race=:rId');
            $stmt->execute(array(
              ':hodds'=>$hOdds, ':refH'=>  $refH, ':rId'=>$refRace
            ));
            $conn=null;
            return true;
        } catch(PDOException $e) {
    ...
    public static function saveNbbyRefH($refH, $nb, $refRace){
        try {
            $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare('UPDATE _horse_tb SET _hnb = :nb WHERE _id_horse=:refH AND _ref_race=:rId');
            $stmt->execute(array(
              ':nb'=>$nb, ':refH'=>  $refH, ':rId'=>$refRace
            ));
            $conn=null;
            return true;
        } catch(PDOException $e) {
    ...
    public static function checkInputRez($refRace){
        try {
            $htype=1;
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _hrank FROM `_horse_tb` WHERE _ref_race=:rId AND _htype=:htype');
          $stmt->execute(array(':rId' => $refRace, ':htype' => $htype));
    ...
    public static function getResultForWPO($refRace){
        try {
            $hrank=1;
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _id_horse, _hnum FROM `_horse_tb` WHERE _ref_race=:rId AND _hrank=:hrank');
          $stmt->execute(array(':rId' => $refRace, ':hrank' => $hrank));
    ...
    public static function getResultPlace($refRace){
        try {
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _id_horse, _hnum FROM `_horse_tb` WHERE _ref_race=:rId AND _hplacerate > 0');
          $stmt->execute(array(':rId' => $refRace));
    ...
    public static function getLossHorses($refRace){
        try {
            $hrank=1;
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    public static function getNonPlHorses($refRace){
        try {
            $plRate=0;
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _id_horse, _hnum FROM `_horse_tb` WHERE _ref_race=:rId AND _hplacerate = :plRate');
          $stmt->execute(array(':rId' => $refRace, ':plRate' => $plRate));
    ...
    public static function getWOPRate($refRace){
        try {
            $htype=12;
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _hWopRate FROM `_horse_tb` WHERE _ref_race=:rId AND _htype = :htype');
          $stmt->execute(array(':rId' => $refRace, ':htype' => $htype));
    ...
    public static function getWebOddsByRaceId($rId){
        try {
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT DISTINCT _hnum, _hodds, _htype FROM `_horse_tb` WHERE _ref_race=:rId AND (_htype=1 OR _htype=11)');
          $stmt->execute(array(':rId' => $rId));

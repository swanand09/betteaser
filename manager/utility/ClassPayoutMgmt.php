...
class PayoutMgmt {
    ...
    public static function getStakeNpo($refRace,$refHorse){
        try {
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT _amount, _payout FROM _payout_tb WHERE _ref_race=:refR AND _ref_horse=:refH");
          $stmt->execute(array(':refR' => $refRace, ':refH'=> $refHorse)); 
    ...
    public static function getTotStakeNpo($refRace){
        try {
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT SUM(_amount) AS _totAmt, SUM(_payout) AS _totPo FROM _payout_tb WHERE _ref_race=:refR");
          $stmt->execute(array(':refR' => $refRace)); 

...
class Bet {
    ...
    public static function getBetForBSlip($lim, $rClient){
        try {
          ...
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          if($lim==1){
              $stmt = $conn->prepare("SELECT _id_bet, _amount, _date, _fold, _curr, _betaccp, _betrnumacc, _gametype FROM `_bet_tb` WHERE _ref_client= :rClient AND _bstatus=0 ORDER BY _date DESC LIMIT 7");

...
class Meeting {
    ...
    function createMeeting(){
        try {
            $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare('INSERT INTO _meeting_tb(_mname, _mdate, _mstatus, _comment, _pitch, _falserail, _display) VALUES(:mname, :mdate, :mstatus, :comment, :pitch, :falserail, :display)');
            $stmt->execute(array(
              ':mname'=>$this->_mname, ':mdate'=>$this->_mdate, ':mstatus'=>$this->_mstatus, ':comment'=>$this->_comment,
                ':pitch'=>$this->_pitch, ':falserail'=>$this->_falserail, ':display'=>$this->_display
            ));
            $conn=null;
            return true;
        } catch(PDOException $e) {
    ...
    function editMeeting(){
        try {
            $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare('UPDATE _meeting_tb SET _mname = :mname, _mdate = :mdate, _mstatus = :mstatus, _comment = :comment, _pitch = :pitch, _falserail = :falserail, _display = :display WHERE _id_meeting = :eid');
            $stmt->execute(array(
              ':mname'=>$this->_mname, ':mdate'=>$this->_mdate, ':mstatus'=>$this->_mstatus, ':comment'=>$this->_comment, 
               ':pitch'=>$this->_pitch, ':falserail'=>$this->_falserail, ':display'=>$this->_display, ':eid'=>  $this->_id_meeting
            ));
    ...
    public static function getCurrentMeeting($id){
        try {
          $conn = new PDO('mysql:host='.DB_HOST.';dbname='.DB, DB_USN, DB_PWD);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare('SELECT _mname FROM _meeting_tb WHERE _id_meeting = :id AND _current=1');
          $stmt->execute(array(':id' => $id));
          $result = $stmt->fetchAll();
          $conn=null;
          return $result;

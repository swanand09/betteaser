(function(i){
    i.fn.weatherfeed=function(o,h,t){
        ...
        return this.each(function(m,q){
            ...
            now=new Date;
            l="http://query.yahooapis.com/v1/public/yql?q="+encodeURIComponent("select * from weather.forecast where "+
                (h.woeid?"woeid":"location")+" in ("+j+") and u='"+h.unit+"'")+"&rnd="+now.getFullYear()+now.getMonth()+now.getDay()+now.getHours()+"&format=json&callback=?";
            i.ajax({
                type:"GET",
                url:l,
                dataType:"json",
                success:function(f){
                    if(f.query){
                        if(0<f.query.results.channel.length)for(var c=f.query.results.channel.length,e=0;e<c;e++)u(q,f.query.results.channel[e],h);else u(q,f.query.results.channel,h);
        ...
        var u=function(f,c,e){
            ...
            var city="Port-Louis, MAUTITIUS";
            e.image&&(b+=' style=" height: 110px; background-image: url(http://l.yimg.com/a/i/us/nws/weather/gr/'+c.item.condition.code+daynight.substring(0,1)+'.png); background-repeat: no-repeat; margin-top: -10px; padding-top:10px; margin-bottom:0px; padding-bottom:0px;"');
            b=b+
            ">"+('<div class="weatherCity" style="font-weight: bold; margin-top: 10px;">'+city+"</div>");
            ...
            e.highlow&&(b+='<div class="weatherRange"><br/><b>High: </b>'+g.high+"&deg; <b>Low: </b>"+g.low+"&deg;</div>");
            e.wind&&(b+='<div class="weatherWind"><b>Wind: </b>'+a+" "+c.wind.speed+c.units.speed+"</div>");
            ...
            if(e.forecast){
                ...
                e.link&&(b+='<div class="weatherLink"></div>');
            p="odd"==p?"even":"odd";
            f.append(b+"</div>")
            },
        s=function(f){

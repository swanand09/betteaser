<?php 
    ...
    include("./utility/ClassClientStatement.php");
                ...
                <div id="racebookContent">
                    <div id="testRace">
                        ...
                        <div id="raceDetails">
                            ...
                            <div id="allBets">
                                <?php
                                    $clId=$clId[0]['_id_client'];
                                    
                                    $st=ClientStatement::getStatement($clId);
                                    
                                    $tab="<table id='tblAllBets-a' class='display'>";
                                    $tab.="<thead><tr class='hdr'><td>Date</td><td>Description</td><td>Status</td><td>Amount</td><td>Potential Payout</td><td>Payout</td><td>Balance</td></tr></thead>";
                                    $bal=0;

                                    for($i=0;$i<count($st);$i++){
                                        if($st[$i]['desc']=='Opening Balance'){
                                            $bal=$st[$i]['amount'];
                                            $st[$i]['bal']=$bal;
                                        }
                                        if($st[$i]['desc']=='Deposit'){
                                            $bal+=$st[$i]['amount'];
                                            $st[$i]['bal']=$bal;
                                        }
                                        if($st[$i]['desc']=='Withdrawal'){
                                            $bal-=$st[$i]['amount'];
                                            $st[$i]['bal']=$bal;
                                        }
                                        if ((strpos($st[$i]['desc'],'Win') !== false) || (strpos($st[$i]['desc'],'Place') !== false))  {
                                            //echo $s['desc'].'Contain Win or Place<br/>';
                                            $bal=$bal-$st[$i]['amount'];
                                            if($st[$i]['pout']!='-'){
                                                $bal=$bal+$st[$i]['pout'];
                                            }
                                    ...
                                    foreach($st as $s){
                                        $tab.="<tr class='gradeA'>";
                                        $tab.="<td>".date("d M Y H:i", strtotime($s['dat']))."</td>";
                                        $tab.="<td>".$s['desc']."</td>";
                                        $tab.="<td>".$s['bstatus']."</td>";
                                        $tab.="<td>".$s['amount']."</td>";
                                        $tab.="<td>".$s['ppout']."</td>";
